...
func (col Color) DistanceLinearRGB(c2 Color) float64 {
	return col.DistanceLinearRgb(c2)
}
...
func (col Color) BlendRgb(c2 Color, t float64) Color {
	return Color{col.R + t*(c2.R-col.R),
		col.G + t*(c2.G-col.G),
		col.B + t*(c2.B-col.B),
		col.A + t*(c2.A-col.A),
	}
...
func FastLinearRgb(r, g, b float64) Color {
	return Color{delinearize_fast(r), delinearize_fast(g), delinearize_fast(b), 1.0}
}
...
// t == 0 results in c1, t == 1 results in c2
func (col Color) BlendHcl(col2 Color, t float64) Color {
	h1, c1, l1 := col.Hcl()
	h2, c2, l2 := col2.Hcl()
