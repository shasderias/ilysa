...
func MarshalWith(target interface{}, spilloverName string) ([]byte, error) {
	me := reflect.ValueOf(target)
	if me.Kind() == reflect.Ptr {
		me = me.Elem()
	}
	...
	typ := reflect.TypeOf(me.Interface())
	...
	for i := 0; i < me.NumField(); i++ {
		var (
			...
			val         = me.FieldByName(name).Interface()
		)

		if name == spilloverName {
			continue
		}
		...
		if opts.Contains("omitempty") && isEmptyValue(me.FieldByName(name)) {
			continue
		}
		...
		rawMsg := json.RawMessage(marshaled)
		merged[name] = &rawMsg
	}

	if spillover := me.FieldByName(spilloverName); !spillover.IsZero() {
		iter := spillover.MapRange()

		for iter.Next() {
			val, err := json.Marshal(iter.Value().Interface())
			if err != nil {
				return nil, err
			}

			rawMsg := json.RawMessage(val)
			merged[iter.Key().String()] = &rawMsg
		}
	}
