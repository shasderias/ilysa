...
func TestMakeColor(t *testing.T) {
	...
	c_ours, ok := FromColor(c_orig_nrgba)
	r, g, b := c_ours.RGB255()
	...
	c_ours, ok = FromColor(c_orig_nrgba64)
	r, g, b = c_ours.RGB255()
	if r != 123 || g != 45 || b != 67 || !ok {
	...
	c_ours, ok = FromColor(c_orig_gray)
	r, g, b = c_ours.RGB255()
	...
	c_ours, ok = FromColor(c_orig_gray16)
	r, g, b = c_ours.RGB255()
	if r != 123 || g != 123 || b != 123 || !ok {
	...
	c_ours, ok = FromColor(c_orig_rgba)
	r, g, b = c_ours.RGB255()
	if r != 0 || g != 0 || b != 0 || ok {
